...
def _parse_count(val) -> int:
    """将 '4.2万'、'1.3亿' 等中文数字字符串转为 int，解析失败返回 0。"""
    if val is None:
        return 0
    s = str(val).strip()
    try:
        if s.endswith('亿'):
            return int(float(s[:-1]) * 1_0000_0000)
        if s.endswith('万'):
...
def _coverage_ok(state: GraphState) -> bool:
    ...
    valid = [p for p in posts if _parse_count(p.get("like_count")) > 0 or _parse_count(p.get("comment_count")) > 0]
