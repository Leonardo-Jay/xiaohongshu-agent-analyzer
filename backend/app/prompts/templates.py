CLASSIFY_PROMPT = """
你是一名产品舆情分析专家，负责理解用户的查询意图。

用户查询: {query}

请分析此查询并以 JSON 格式返回，只返回 JSON，不要其他文字：
{{
  "intent": "product_quality | price_value | comparison | general",
  "product_entities": ["产品名1", "产品名2"],
  "aliases": ["别名1", "别名2"],
...
SCREEN_PROMPT = """
你是一名产品舆情分析师，负责筛选与用户查询最相关的小红书帖子。
...
判断标准：
1. 帖子内容直接涉及用户查询的产品/话题
2. 有实质性评价内容（非广告、非无效内容）
3. 互动数据较高（点赞/评论多）
...
你是产品舆情分析师，请基于以下小红书用户数据撰写详实的分析报告。
...
- 用 # 作为报告总标题
- 用 ## 划分主要章节：整体印象、优点分析、缺点分析、争议点、总结
- 每个 ## 章节下用 ### 子标题对每个观点单独展开，每个观点至少 4~6 句话
- 描述普遍程度（「多数用户」「部分用户」「普遍存在」）和具体场景（「日常使用中」「长时间使用后」）
- 引用 1~2 句用户原话（来自 evidence_quotes），用引号括起
- 综合建议章节给出 3~5 条可操作的具体建议
- 总字数不少于 600 字
"""
